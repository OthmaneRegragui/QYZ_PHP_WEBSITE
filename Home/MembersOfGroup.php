...
<body>
    <div class="container">
        ...
        <?php
        $sql="SELECT UserId,DateOfIn from TUsersOfGroups WHERE (GroupId='$GroupId') and REQUEST='Accept';";
    ...
    <script>
        function Cancel(GroupId,UserId,UserName){
            ...
            $.post('MembersOfGroup.php',{GId:GroupId,UId:UserId,OP:'Cancel'})
            $(`#${UserName}`).remove()
        }
        function Ban(GroupId,UserId,UserName){
            $('#Alert').attr('class','alert alert-danger')
            $('#Alert').text(`${UserName} Ban From Group`)
            $.post('MembersOfGroup.php',{GId:GroupId,UId:UserId,OP:'Ban'})
            $(`#${UserName}`).remove()
        }
    </script>
</body>
